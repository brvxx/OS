/* programma main1.c che e' sostanzialmente una versione migliorata del programma main.c, quello che cambia e' che andiamo ad eliminare tutti i casi possibili di 
arresto anomalo dei processi figli. Infatti una casistica che non viene considerata nel programma main.c e' il caso in cui uno dei file passati per parametro (NON
IL PRIMO), non esista oppure non sia leggibile. 

Facciamo un esempio concreto per capire:
Ipotizziamo di passare solo due nomi di file al programma, F0 e F1, dove pero' F1 non e' un nome di un file esistente e F0 e' un file dalla prima riga MOLTO lunga.
Quando amndiamo in esecuzione, quello che succede e' che il processo padre crea correttamente prima il processo P0 e poi il processo P1. Il fatto che il file associato 
a P0 abbia una riga molto lunga NON e' un caso, infatti questo richiedera' a P0 molto tempo per poter contare le occorrenze, abbastanza tempo per far si' che l'esecuzione
concorrente di P1 riesca a riconoscere che F1 non esista provocandone dunque la sua terminazione. A questo punto quando P0 provera' a scrivere sul lato di scrittura della 
prima pipe (che collega P0 a P1) siamo CERTI che P1 sia oramai terminato, quindi quello che sta facendo implicitamente P0 e' scrivere su una pipe senza lettore. Questo 
comportamento ANOMALO viene detectato dal OS che inviera' il SIGPIPE a P0, causando la sua terminazione anomala.

Dal momento che per file con righe lunghe questo e' un problema pressoche' inevitabile, quello che possiamo fare per risolverlo e' sostanzialmente andare a IGNORARE IL 
SIGPIPE tramite la primitiva SIGNAL. Cosi' facendo anche se P0 va a scrivere sulla pipe1 quando P1 e' gia' terminato, l'avvenimento non causera' la sua terminazione 
anomala

Nota poi che sebbene noi andiamo ad ignorare il SIGPIPE, comunque il kernel opera in modo tale da far FALLIRE la write sulla pipe senza lettore. Quindi sebbene P0 NON 
termini in modo anomalo, comunque NON riuscira' a scrivere sulla pipe1 */
...
#include <string.h> 
#include <ctype.h> 
#include <signal.h>     /* aggiunta della libreria per la gestione dei segnali */
