/* programma C main.c che risolve la parte di C della prova di esame totale tenutasi il 11 luglio 2018 (versione con calcolo del minimo da parte del padre) */ 
...
#include <string.h> 
#include <ctype.h> 
#include <limits.h>     /* mentre nella versione col calcolo del massimo, per inizializzare il valore di max_pos bastava attribuirle un valore negativo, dal momento che sicuramente 
                            neanche nel caso in cui la prima occorrenza di CZ fosse stata per tutti i file nella prima posizione, si poteva ottenere un valore cosi' piccolo per la pos.
                            Qui la faccenda e' un pochetto piu' delicata, infatti abbiamo bisogno di un numero ESTREMAMENTE GRANDE, cosi' che qualisasi posizione riceviamo da parte dei 
                            figli sicuramente sara' minore di questa e possa inizializzare il valore di min_pos. 
                            Per farlo useremo appunto la libreria limits.h che contiene la definizione della macro LONG_MAX, ossia il massimo valore rappresentabile con un long int. E' 
                            come se fosse il +inf, e' IMPOSSIBILE dunque che l'occorrenza di un carattere possa trovarsi una posizione cosi' grande, il file dovrebbe essere dell'ordine 
                            di grandezza di 10^19 byte XD */
...
int main (int argc, char **argv)
{
	...
	long int min_pos; 	/* posizione minima all'interno di un insieme di posizioni spedite dai figli */ 
	int min_idx; 		/* indice del processo figlio al quale si attribuisce la posizione piu' piccola dell'occorrenza */
				...
				{ 	
					...
					{ 	
						...
						printf("Il processo figlio di indice %d e pid = %d ha trovato l'occorrenza no. %d del carattere '%c' nel file %s "
							"nella posizione minima rispetto agli altri figli, ossia %ld\n", i, getpid(), occ, CZ, argv[i + 2], pos); 
					} 
				} 
	...
	/* ciclo di recupero ordinato delle posizioni spedite dai figli. Si va avanti a recuperare le posizioni e a determinare quella minima, fintanto che TUTTI i processi 
	non sono terminati */ 
	while(!all_ended()) 
	{
		min_pos = LONG_MAX; 	/* inizializziamo la posizione minima ad un valore altissimo, per il quale, qualsiasi valore di posizione tornato dai figli risulti minore */
		...
		{ 
			...
			{ 
				...
				/* controllo se si ha trovato la posizione minima*/ 
				if (pos < min_pos) 
				{ 
					min_pos = pos; 
					min_idx = i;
		...
		{ 
			...
			else 	/* figlio che NON ha trovato la posizione minima nell'occorrenza corrente */ 
			{ 
				ch_ctrl = 'N'; 
			} 
