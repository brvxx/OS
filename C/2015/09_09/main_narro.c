...
cstatus_t confronto[MAX_CHLD] = {STATUS_RUNNING}; 
			...
			{ 
				if(confronto[i] == STATUS_RUNNING)
				{
					...
					else
					{
						...
						confronto[i] = STATUS_EXITED;	 /* effettivamente e' vero che non si e' neanche fatto il confronto, ma cosi' facendo evitiamo ad ogni iterazione di andare a provare a comunicare con
		...
		{ 
			switch (confronto[i])
			{
				...
				case STATUS_INVCHR:{
					...
					if (ret != 0)
					{
						printf("[ERROR] invio del SIGKILL non riuscito\n");
					}
					break; 
				}

				case STATUS_EXITED:{
					printf("[DEBUG] il processo di indice %d e' stato terminato autonomamente\n", i);
					break;
				}
			}
		}
